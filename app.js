const http = require('http');//incluyo.el.módulo http
const herramienta = require('./src/home')
const fs = require('fs');
const prueba= require('./src/cartelera');
const puntaje= require('./src/masvotadas');
const salas= require('./src/salas');
const lasPreguntas= require('./src/preguntas')


//const dm = requiere('nodemon');---> Nodemon es una utilidad que monitorea los cambios en el código fuente que se esta desarrollando y automáticamente re inicia el servidor. Es una herramienta muy útil para desarrollo de aplicaciones en nodojs.

const movies = [ //arreglo de objetos..si la variable no la voy a modificar en un futuro,es constante
	{
...
//let voto = puntaje.peliculas(movies);
//console.log(voto);



//let titulos= herramienta.peliculas(movies);
//console.log(titulos);
//console.log(Object.values(movies));

			
// peliListas += oneMovies + '\n';
//let peliListas = '\n';

let salitas= salas.nuestrasSalas(theaters);



// Servidor
http.createServer((req, res) => {
	res.writeHead(200, { 'Content-Type': 'text/plain; charset=utf-8' });//conocer los códigos que devuelve el servidor..si marca 200 esta todo ok,404 para cvambiar página..500 el error es del lado del servidor(error en el backend)..400 error en el frontend
	...
	switch (req.url) {
		...
		case '/':
               		let titulos= herramienta.peliculas(movies);
		     		//console.log(titulos);
					let totalPeliculas= titulos.length;
					//console.log(totalPeliculas);
					//let ancla=herramienta.anclas;
				res.end(`${herramienta.titulo} \n \n ${titulos} \n \n El total de pelicula son: ${totalPeliculas}\n \n Pie de Página \n En Cartelera \n Más votadas \n Sucursales \n Contacto \n Preguntas Frecuentes\n\n`);
			 //la peticion viaja por get, porque lo pedimos por la url
			break;
		// En cartelera
		case '/en-cartelera':
			let cartelera= prueba.activas(movies);
			let orden='\n';
			for(datos in cartelera){
				orden +=cartelera[datos] + '\n';
			}
			res.end(`${orden}`);
			break;
		case '/mas-votadas':
			let masVotadas= herramienta.peliculas(movies);
			let totalPeliculasCartelera= masVotadas.length;
			let voto = puntaje.promedio(movies);
			let peliculas = puntaje.peliculas(movies);
			res.end(` Más Votadas \n\n Total de Peliculas en Cartelera: ${totalPeliculasCartelera} \n\n Rating promedio: ${voto} \n\n Listados de películas: ${peliculas}`);
			break;
		case '/sucursales':
			let totalSalas= salas.totalSalas;
			let salitas= salas.nuestrasSalas(theaters);
			res.end(`Título​: ​Nuestras Salas \n\n Total de salas: ${totalSalas}.\n\n Listado de Salas\n\n ${salitas}`);
			break;
		case '/contacto':
		...
		case '/preguntas-frecuentes':
			let dudas= lasPreguntas.listado(faqs);
